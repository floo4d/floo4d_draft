...
'''
Processing for mperforming urQRd and/or Fista on 2D FTICR datasets.
...
def _do_proc_F1_flip_modu(data):
    ...
    d.axis1.itype = 0
    d.axis2.itype = 1
    if parameter.samplingfile != None:      # NUS ?
        ...
        if parameter.samplingfile_fake:    # samplingfile_fake allows to fake NUS on complete data-sets
            d.set_buffer( d.get_buffer()[d.axis1.sampling])     # throw points
        #print "d.size1",d.size1
        #print "d.size2",d.size2
        d.zf()                          # add missing points by padding with zeros
        #print "d.size1",d.size1
        #print "d.size2",d.size2
        # this is required by the flipphase() below
        #print "d.get_buffer().shape",d.get_buffer().shape
    d.flipphase(0.0, 180*shift) # equivalent to  d.flip()  d.phase()  d.flop()
    #print "d.get_buffer().shape after flip",d.get_buffer().shape
    if parameter.do_urqrd:
        d.urqrd(k = parameter.urqrd_rank, axis = 1)#

    if parameter.samplingfile != None:      # NUS ?
        #d.axis1.load_sampling(parameter.samplingfile)
        if parameter.do_recital:
            print 'throwing points away'
            print "type(d.get_buffer())",type(d.get_buffer())
            print "type(d.axis1.sampling)",type(d.axis1.sampling)
            print "d.get_buffer()[d.axis1.sampling].shape",d.get_buffer()[d.axis1.sampling].shape
            d.set_buffer( d.get_buffer()[d.axis1.sampling])     # throw points created before
        # else:    # if NUS but not recital, put zeros on unmeasured points
        #     d.zf()
    #print "d.get_buffer().shape just before recital",d.get_buffer().shape
    if parameter.do_recital:
        for nbcol in [0,1]:
            e = FTICRData(buffer = d.col(nbcol).get_buffer().copy())
            e.recital(size, miniteration = parameter.recital_miniterations,
                iterations = parameter.recital_iterations, scale_noise = parameter.recital_scale_noise )
            d.col(nbcol).set_buffer(e.get_buffer())
    else:
        apod(d, size, axis = 1)
...
def do_proc_F1_flip_modu(dinp, doutp, parameter, nproc = None):
    ...
    if mpiutil.MPI_size > 1:      # MAD : KLUDGE to code for MPI processing in nproc
        nproc = 0
    if nproc == None:
...
class Proc_Parameters(object):
    ...
    def __init__(self, configfile = None):
        ...
        self.zflist = None
        self.szmlist = None
        self.do_recital = False
        self.recital_miniterations = 100
        self.recital_iterations = 1
        self.recital_scale_noise = 1.0
        self.mp = False
        self.nproc = 4
        # files
        self.apex = None
    ...
    def load(self, cp):
        ...
        self.largest = cp.getint( "processing", "largest_file", LARGESTDATA)            # largest allowed file
        ...
        self.do_recital = cp.getboolean( "processing", "do_recital", str(self.do_recital)) # do_recital
        self.recital_miniterations = cp.getint( "processing", "recital_miniterations", self.recital_miniterations)
        self.recital_iterations = cp.getint( "processing", "recital_iterations", self.recital_iterations)
        self.recital_scale_noise = cp.getfloat( "processing", "recital_scale_noise", self.recital_scale_noise)
        self.do_rem_ridge = cp.getboolean( "processing", "do_rem_ridge", str(self.do_rem_ridge))
        self.mp = cp.getboolean( "processing", "use_multiprocessing", str(self.do_rem_ridge))
        ...
        zflist =  cp.get( "processing", "zerofilling", "")                              # get zf levels
        if zflist:
            ...
            self.zflist = None
        szmlist =  cp.get( "processing", "sizemultipliers", "")                          #  get szm levels
        if szmlist:
        ...
        if self.samplingfile and self.do_urqrd and self.do_recital:
...
def main(argv = None):
    ...
    if param.do_F1:
        ...
        group = 'resol1'
        if param.do_recital:    # recital creates highly compressible files
