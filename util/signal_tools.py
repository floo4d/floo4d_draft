...
class SIGNAL_NOISE(object):
    ...
    def __init__(self, lenfid, nbpeaks, amplitude, noise, LB = 1.11,\
        shape = "triangular", noisetype = 'additive', shift = 0,\
                realfid = False, trunc = None, seed = True):
        '''
        ...
        if seed:
            np.random.seed(11232)                    # frequency shift
...
class MULTITESTS(object):
    '''
    Class for testing algorithm on many synthetic datasets.
    Algo parameters must not be keyword parameters.
    ### parameters
    algo : algorithm used.
    ampl_sig : amplitude for the synthetic signal generated
    ampl_noise : amplitude for the noise in the synthetic signal generated
    nbpeaks : number of frequencies in the signal
    nb_tries : number of generated synthetic signals for testing the algo.
    '''
    
    def __init__(self, algo = None, lenfid = 10000, ampl_sig = 50,\
            ampl_noise = 80.0, nbpeaks = 6, nb_tries = 5,\
            npk = False, plot_fft = True, trunc = None):
        
        self.lenfid = lenfid            # length of the Fid
        self.algo = algo                # algo to be tested.
        self.ampl_sig = ampl_sig        # amplitude of the signal
        self.ampl_noise = ampl_noise    # amplitude of the noise
        self.nbpeaks = nbpeaks          # number of frequencies
        self.nb_tries = nb_tries        # number of generated Fids for testing the algo.
        self.npk = npk               # use NPK syntax
        self.plot_fft = plot_fft    # make the FFT or plot directly the result.
        self.trunc = trunc
        self.shift = 0              # Shift of all the frequencies
        self.make_plot = True      # show plots with noisy data, denoised etc.. 
        self.make_all = True
        
        from util.dynsubplot import subpl 
        self.subpl = subpl
        import Display.testplot as testplot
        self.plt = testplot.plot()

    def run(self, algo_param):
        '''
        Launch the algorithm on the Fids.
        You have to give the algo parameters "algo_param" as a dictionary.
        '''
        for self.essai in range(self.nb_tries):
            print "essai number ", self.essai
            if self.make_all:
                self.ampl = self.ampl_sig*np.fabs(np.random.randn(self.nbpeaks)) # Make random amplitudes in the signal. 
                self.signoise = SIGNAL_NOISE(self.lenfid, self.nbpeaks,
                    self.ampl, self.ampl_noise, shape = 'list', seed = False, trunc = self.trunc)
                if self.trunc:
                    self.signoise.cut() # passing to truncated signal
                if self.npk:
                    npk_sig = NPKData() # make NPK object
                    npk_sig.set_buffer(self.signoise.fid)
                    #print "algo_param.values() ", algo_param.values()
                    print type(npk_sig)
                    #param_str = str(algo_param.values()).replace('[','').replace(']','')
                    #param_str = '1e5'
                    param_str = algo_param
                    command_line = 'npk_sig.'+ self.algo + '({})'.format(param_str) # + '.fft().modulus().display()' #show = True
                    print "command_line ", command_line
                    t0 = time.time()
                    eval(command_line)
                    t_algo = time.time()-t0
                    if self.algo == 'recital':
                        self.data_algo = np.roll(npk_sig.get_buffer()[::-1],1)
                    else:
                        self.data_algo = npk_sig.get_buffer()
                else:
                    t0 = time.time()
                    #print "algo_param.values() ", algo_param.values()
                    self.data_algo = self.algo(self.signoise.fid, eval(algo_param))                  # applying algo to the data
                    t_algo = time.time() - t0
                
                iSNR = self.signoise.iSNR
                print "Initial Noisy Data SNR: %.2f dB "%(iSNR)
                self.fdata_algo  = mfft(self.data_algo)# FFT of urQRd denoised signal
                if self.plot_fft:
                    self.data_plotted = self.fdata_algo # plot the spectrum
                else:
                    self.data_plotted = abs(self.data_algo) # take the module for plotting the fid.
                fSNR = self.signoise.SNR_dB(self.data_algo)
                print "Denoised SNR: %.2f dB  - processing gain : %.2f dB"%( fSNR, fSNR-iSNR )
                print "processing time for urQRd : %f sec"%t_algo
            self.plot()   # plot the result of the algo on the data.

    def plot(self):
        '''
        Makes plot corresponding to each try.
        '''
        def plot_algo(sub, self):
            if self.npk:
                sub.plot(self.data_plotted ,'r', label = self.algo)          # plot the spectrum  after denoising
            else:
                sub.plot(self.data_plotted ,'r', label = self.algo.__name__)   # essai_num0.jpg      
        if self.make_plot :
            sub = self.subpl(2, self.plt)
            sub.title('essai num ' + str(self.essai))
            sub.next()
            sub.plot(self.signoise.fid,'k', label = "noisy signal")             # plot the noisy fid
            sub.next()
            plot_algo(sub, self)                                             # plot the spectrum after denoising
            self.signoise.fid = self.signoise.zerof(self.signoise.fid, self.data_plotted)
            sub.plot(self.signoise.spec,'k', label = "noisy spectrum")          # plot the noisy spectrum
            sub.next()
            plot_algo(sub, self)                                             # plot the spectrum after denoising
            sub.next()
            sub.plot(self.signoise.spec0 ,'b', label = 'original ')             # plot the spectrum wihtout noise
            sub.show()
