...
class ZOOM3D(object):
    ...
    def plot3dset(self, ax, visible, X):
        '''
        Settings of the ticks and labels for the 3D view.
        '''
        plt.setp(ax.get_zticklabels(), visible = visible)                                           # z ticks labels                                          # x ticks labels
        plt.setp(ax.get_yticklabels(), visible = visible)                                           # y ticks labels
        ticksx = np.round(np.linspace(X.min(), X.max(), 8)[::-1], 1)
        ax.set_xticklabels(ticksx) 
        ax.set_xlabel('m/z(1)')                                                                     # label m/z axis 1
        ax.set_ylabel('m/z(2)')                                                                     # label m/z axis 2
        ax.set_zlabel('Intensity')                                                                  # label for the intensity.
    ...
    def plotregion3d(self, d, pt1min, pt1max, pt2min, pt2max, visible = False):
        ...
        fig = plt.figure()
        stride = 1
        ax = fig.add_subplot(111, projection = '3d')                                                # defines 3D matplotlib axes.
        X,Y, Z = self.makemesh(d, pt1min, pt1max, pt2min, pt2max)                                   # makes the meshgrid
        print "makes mesh OK "
        if self.plot_3D_ok : 
            surf = ax.plot_surface(X[:,::-1], Y, Z, rstride = 1, cstride = 1,                       # makes the 3d plot.
                    cmap = cm.coolwarm, linewidth = 0.5, antialiased = True)                        #
            self.plot3dset(ax, visible, X)                                                          # adds the labels
    
    def rect(self, pt1min, pt1max, pt2min, pt2max, alpha = 0.3):
        '''
        Used in plotcont for showing the zone to be plot in 3D.
        '''
        size1 = pt1max - pt1min
        size2 = pt2max - pt2min
        pos = (pt1min, pt2min) 
        self.rect = plt.Rectangle(pos, size1, size2, alpha = alpha)                                 #, facecolor="#aaaaaa"
        self.rect.set_color('r')
        plt.gca().add_patch(rect)
    
    def plotcont(self, data, pt1min, pt1max, pt2min, pt2max):
        '''
        Shows the area used for 3D representation.
        '''
        if debug(self):
            print "in plotcont"
        size1, size2 = data.buffer.shape
        print size1, size2
        fig = plt.figure()
        portion = 1
        c1min, c1max, c2min, c2max = 0, size2/portion, 0, size1/portion 
        if debug(self):
            print "c1min, c1max, c2min, c2max ", c1min, c1max, c2min, c2max 
        step = 1
        level = [1e5, 1e6]
        zonetoplot = data.buffer[c1min:c1max:step, c2min:c2max:step]
        plt.contour(np.arange(c2min, c2max, step),\
            np.arange(c1min, c1max, step), zonetoplot, level)
        self.rect(pt2min, pt2max, pt1min, pt1max)
    
    def contourzoom3d(self, d, pt1min, pt1max, pt2min, pt2max):
        if debug(self):
            print "in contourzoom3d"
        self.plotcont(d, pt1min, pt1max, pt2min, pt2max)
