...
#from Algo.urQRd_matrix import rQRd as urQRd
...
def _do_proc_F1_flip_modu(data):
    ...
    if parameter.samplingfile != None:      # NUS ?
        ...
        if not parameter.do_recital:    # if NUS but not recital, put zeros on unmeasured points
            d.zf()
    if parameter.do_recital:
        for nbcol in [0,1]:
            d.col(nbcol).recital(size, miniteration = parameter.recital_miniterations,
...
class Proc_Parameters(object):
    ...
    def load(self, cp):
        ...
        if zflist:
            ...
            self.zflist = None
        szmlist =  cp.get( "processing", "sizemulipliers", "")                          #  get szm levels
        if szmlist:
