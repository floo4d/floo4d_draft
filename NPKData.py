...
#            print self.buffer.base.flags['UPDATEIFCOPY']
    ...
    def plus(self):
        ...
        see also :  minus, zeroing, zero, one
        """
...
#                    if i%16==0: print i,self.sizeZ
    ...
    #---------------------------------------------------------------------------
    def recital(self, finalsize, iterations = 2, miniteration = 100, noise = None, scale_noise = 1.0, mfista = False):
        """
        ...
        """
        # if data are real
        #   sampled (size P out of N), : D
        #   or not
        # spectrum is complex : S (size M >N )
        # 
        # if sampled :
        # transform() :  S.irfft().chsize(N).sampling()  ->  P real
        #   and then
        # ttransform() :  D.zf().chsize(M).rfft() ->    M complex
        #
        # else
        # transform() :  S.irfft().chsize(N)  ->  P real
        #   and then
        # ttransform() :  D.chsize(M).rfft() ->    M complex
        
        from Algo.Fista import Fista
        import Algo.CS_transformations as CS_transformations
        if self.dim >1:
            raise NPKError("Not implemented yet", data = self)
        size_observed = self.size1/(2**self.itype)
        tr = CS_transformations.transformations(finalsize, size_observed,  debug = self.debug)
        tr.sampling = self.axis1.sampling    # either None of sampling func
        if self.itype == 0: # fid is real
            tr.tft = _tt
            tr.ft = _t
        elif self.itype == 1:   # fid is complex
            tr.tft = lambda x: as_cpx(_base_fft(as_float(x)))
            tr.ft = lambda x: as_cpx(_base_ifft(as_float(x)))
        tr.check()
        if noise:
            self.fista = Fista(tr.transform, tr.ttransform, self.get_buffer(), noise_level = noise, mfista = mfista)
        else:
            self.fista = Fista(tr.transform, tr.ttransform, self.get_buffer(), scale_noise = scale_noise, mfista = mfista)
        self.fista.iterat = iterations
#        self.fista.prec = 0.1
        self.fista.miniterat = miniteration
#        self.debug = 1
        if self.debug >0:
            self.fista.solve_debug = True
            self.fista.report()              # report Fista parameters.
            self.fista.adaptlamb_debug = True
        b = self.fista.solve()       # launches the algorithm.
        self.set_buffer( b )
        if self.debug >0:
            self.fista.plot_monitorings()
        return self
    #---------------------------------------------------------------------------
    ...
    def hypercomplex_modulus(self):
        
        b = np.zeros((self.size1/2, self.size2/2))
        brr = self.buffer[::2, ::2]
        bri = self.buffer[::2, 1::2]
        bir = self.buffer[1::2, ::2]
        bii = self.buffer[1::2, 1::2]
        ...
        if self.dim == 1:
            ...
            if self.axis1.itype == 0 and self.axis2.itype == 0:      # has to be complex
                ...
                if self.axis1.itype == 1 and self.axis2.itype == 0: # along F1 only
                    ...
                    self.axis1.itype = 0
                    self.axis2.itype = 0
                    #print "makes hypercomplex_modulus"
                    b =  self.hypercomplex_modulus()
                self.buffer = b
        elif self.dim == 3:
            raise NPKError("reste a faire")
        self.absmax = 0.0
...
class NPKDataTests(unittest.TestCase):
    ...
    def test_NUS_sampling(self):
        ...
        '''
        import util.signal_tools as u
        from numpy.fft import fft
        e = NPKData(dim = 1)
        e.axis1.load_sampling("../DATA_test/Sampling_file.list")
        size = 20000
        sig = u.fid_signoise(10, 100, size, 50, shift = 7000)
        echant = sig[e.axis1.sampling]
        print "echant.size ",echant.size
        f = NPKData(buffer = echant)
        f.axis1.load_sampling("../DATA_test/Sampling_file.list")
        g = f.copy()
        h = f.copy()
        g.zf()
        g.urqrd(40).fft().modulus().display(label = "NUS : sampling noise removed by urQRd")
        f.zf()  # zf adds zero on missing points for sampled data-sets
        f.fft().modulus().display(label = "NUS : FFT with sampling noise")
        #h.debug=1
        t0 = time.time()
        h.recital(size, miniteration = 1200, iterations = 1, scale_noise = 1) #4800
        print "time for recital is {} ".format(time.time()-t0)
        h.modulus().display(show = True, label = "NUS : processed with recital", new_fig = False)

    def test_recital_synthetic(self):
        """
        Test recital on synthetic dataset.
        """
        d = NPKData(buffer = np.zeros((10000,)), debug = 0)  # set-up a scene
        d.axis1.specwidth = 20000.0
        d[1233] = 10000.0
        d[1333] = 10000.0
        d[3333] = 10000.0
        d.ifftr()
        d.chsize(1000).real()
        d.addnoise(1.0)
        d.copy().chsize(10000).rfft().modulus().display(label = 'FFT with noise')
        t0 = time.time()
        d.recital(10000, miniteration = 1200, iterations = 1) #2400
        print "time for recital is {} ".format(time.time()-t0)
        d.modulus().display(label = 'Recital processing', show = True, new_fig = False)
    
    def test_superresolution(self):
        '''
        NUS example
        removing the sampling noise white_noise.jpg
        '''
        import util.signal_tools as u
        from numpy.fft import fft

        size = 20000
        sig = u.fid_signoise(10, 100, size, 2000, shift = 7000)
        f = NPKData(buffer = sig)
        f.units = "Hz"
        g = f.copy()
        f.fft().modulus().display(label = "FFT")
        g.recital(size*4, miniteration = 300, iterations = 1)
        g.modulus().display(label = "Recital with 4x sur resolution", show = True, new_fig = False)
