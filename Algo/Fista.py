...
plt = testplot.plot()
from util.counter import counting, timeit # decorators for timing, counting etc.. 
from util.debug_tools import dec_class_pr, decclassdebugging, debug, pr # debugging
from util.signal_tools import findnoiselevel_offset as findnoiselevel # find the noise level
#libraries for test. 
import unittest
...
class Fista(object):
    '''
    b = Ax
    b is the result of measurement
    ...
    def __init__(self, transf, ttransf, b, scale_noise = 1.0,\
        ...
        del(temp)
        self.divide_noiselevel = 1./scale_noise        # divides the noiselevel.                      
        ...
        self.grad = None                                    # Gradient for Fista
        self.xtargm1 = None                                 #
        ...
        #### Lambda
        if noise_level == None:
            self.noise_level = findnoiselevel(abs(self.ttransf(self.b)), nbseg = 11)/self.divide_noiselevel # The noise level used for finding the first lambda.
        else: 
    ...
    def F(self, y):
        ...
        '''
        return norm(self.transf(y) - self.b)**2-self.lambdaparam*abs(y).sum()
    ...
    def chi2_test(self):
        ...
        if self.best_precision == None or current_precision < self.best_precision : # test of precision 
    ...
    def fistaloop(self):
        ...
        while 1 : 
            ...
            if not self.recomputlamb :
                ...
                self.chi2 = (norm(self.b-self.transf(self.yk))**2)/self.noiselevelb**2          # calculates the chi2.
                if debug(self):
    ...
    def corealgo(self):
        ...
        self.grad = self.ttransf(interm_residual)                               # Gradient part, returning to image domain
        if debug(self):
        ...
        '''
        new = self.yk - 2.0*self.t*self.grad                                    # new step before Thresholding
        self.chi2interm = (norm(interm_residual)**2)/self.noiselevelb**2        # calculate an intermediate chi2.
        if debug(self):
            print "intermediate chi2 in corealgo", self.chi2interm/self.b.size
        if self.chi2interm/self.b.size > 1 :                                    # if not overshooting continue the calculus 
            if self.mfista:
                self.ztarg = self.Thresh_shrink(new)                    # calculation of self.ztarg for MFISTA
            else:
    ...
    def plot_lambda(self):
        ...
        plt.plot(self.monitor_lambda)                                           # plots values of lambda during Fista execution.
    ...
    def plot_chi2(self):
        ...
        for i, fol in enumerate(self.all_monitor_chi2):
            ...
            plt.ylabel("$\chi$2")
            plt.plot(fol, label = "$\lambda=$" + str(round(self.monitor_lambda[i], 2)))  # plots evolution of chi2 for one lambda value.
        plt.legend()
...
class FistaTests(unittest.TestCase):
    ...
    def test_superresol_synthetic_data_Fista(self):
        '''
    ...
    def test_superresol_DATA_test_Fista(self):
        '''
        Fista on synthetic data for testing superresolution.
        ...
        ###
        superresol = 4
        f = Import_2D('../DATA_test/cytoC_2D_000001.d')
        f.report()
        f.units = 'm/z'
    ...
    def test_superresolution_modif_Thresholding_Fista(self):
        '''
        Fista on synthetic data for testing superresolution.
        '''
    ...
    def test_sampling_Fista(self):
        ...
        ######
        c = NPKData(buffer = b[sampling].copy())                                                    # produces an NPKdata sampled with sampling informations.
